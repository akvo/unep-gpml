...
(deftest handler-profile-image
  (testing "Image is available"
    (let [system (ig/init fixtures/*system* [::image/profile])
          handler (::image/profile system)
          db (-> system :duct.database.sql/hikaricp :spec)
          _ (db.stakeholder/new-stakeholder-picture db {:picture picture})
          resp (handler (-> (mock/request :get "/image/profile/")
                            (assoc :parameters {:path {:id 1}}
                                   :path-params {:id 1}
                                   )))]
      (is (= 200 (:status resp))))))

(deftest handler-profile-image-not-found
