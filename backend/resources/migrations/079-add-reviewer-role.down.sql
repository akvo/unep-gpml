...
-- NOTE: Something like this could work, if we have the permissions to
-- edit the pg_enum table, but we don't!
-- DELETE FROM pg_enum WHERE oid IN (
--   SELECT enum.oid FROM pg_enum enum
--     JOIN pg_type type ON
--       type.oid = enum.enumtypid AND
--       type.typname = 'stakeholder_role' AND
--       enum.enumlabel = 'REVIEWER'
-- );
