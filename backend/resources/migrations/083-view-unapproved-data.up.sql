...
-- Topic view with all (including unapproved) topics
ALTER VIEW IF EXISTS v_topic RENAME TO v_topic_all;

-- Topic view
CREATE OR REPLACE VIEW v_topic AS
 SELECT * FROM v_topic_all t
  WHERE t.json->>'review_status' = 'APPROVED'
  ORDER BY (t.json->>'created')::timestamptz;
