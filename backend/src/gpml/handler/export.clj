...
(defn- get-export-values [entity-coll entity-key-map sorted-entity-columns]
...
(defn- export-users [{:keys [db] :as config} review-status]
...
(defn- export-entities [{:keys [db] :as config} review-status]
...
(defn- export-non-member-entities [{:keys [db] :as config} review-status]
  (let [src-dst-files-keys-mapping {:logo_id :logo}
        non-member-entities
...
(defn- export-topics [{:keys [db] :as config} review-status]
