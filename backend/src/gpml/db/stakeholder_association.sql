...
stakeholder_association AS (
    SELECT
        ...
        concat_ws(' ', s.first_name, s.last_name))) AS stakeholder_connections,
    ...
    SELECT
        ...
        concat_ws(' ', s.first_name, s.last_name))) AS stakeholder_connections,
        json_agg(json_build_object('id', oe.id, 'entity_id', oe.organisation, 'role', oe.association, 'entity', o.name)) AS
        entity_connections
    FROM
    ...
    SELECT
        ...
        concat_ws(' ', s.first_name, s.last_name))) AS stakeholder_connections,
        json_agg(json_build_object('id', ot.id, 'entity_id', ot.organisation, 'role', ot.association, 'entity', o.name)) AS
        entity_connections
    FROM
    ...
    SELECT
        ...
        concat_ws(' ', s.first_name, s.last_name))) AS stakeholder_connections,
        json_agg(json_build_object('id', op.id, 'entity_id', op.organisation, 'role', op.association, 'entity', o.name)) AS
        entity_connections
    FROM
    ...
    SELECT
        ...
        concat_ws(' ', s.first_name, s.last_name))) AS stakeholder_connections,
        json_agg(json_build_object('id', oi.id, 'entity_id', oi.organisation, 'role', oi.association, 'entity', o.name)) AS
        entity_connections
    FROM
