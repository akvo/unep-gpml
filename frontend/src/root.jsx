...
const disclaimerHome = (
  <>
    The GPML Digital Platform Phase 1 is now live and currently a Beta Version. Help us test the platform and let us know what you think at{" "}
...
const disclaimerBrowse = (
  <>
    UNEP shall not be liable for third party content hosted on the platform. Contact us if you have any concerns with the content at:{" "}
    <a style={{ color: "white" }} href="mailto:unep-gpmarinelitter@un.org">
      unep-gpmarinelitter@un.org
    </a>
...
const Root = () => {
  const { isAuthenticated, getIdTokenClaims, loginWithPopup, logout, user } = useAuth0();
  ...
  return (
    <Router>
      <div id="root">
        ...
        />
        <Route path="/browse" render={props => <Browse {...props} profile={profile} countData={data} setSignupModalVisible={setSignupModalVisible} updateDisclaimer={updateDisclaimer} filters={filters} setFilters={setFilters} countries={countries} />} />
        <Route path="/add-event" render={props => <AddEvent {...props} updateDisclaimer={updateDisclaimer} countries={countries} />} />
        <Route path="/profile" render={props => <ProfileView {...{ ...props, profile, tags, setProfile, countries }} updateDisclaimer={updateDisclaimer} />} />
        <Route path="/signup" render={props => <SignupView {...props} updateDisclaimer={updateDisclaimer} />} />
        <Route path="/:type/:id" render={props => <DetailsView {...props} profile={profile} setSignupModalVisible={setSignupModalVisible} updateDisclaimer={updateDisclaimer} countries={countries} />} />
        <Footer />
      </div>
      <SignupModal visible={signupModalVisible} onCancel={() => setSignupModalVisible(false)} {...{ tags, setProfile, countries, profile }} />
  ...
  if (isAuthenticated) {
    ...
    return (
      <Button
        ...
        size="large"
        onClick={e => {
          Object.keys(profile).length !== 0 ? setEventWarningVisible(true) : setSignupModalVisible(true);
        }}
      >
        + Add
      </Button>
