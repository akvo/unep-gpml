...
const sectionKey = 'data-available-tools'
...
const View = ({ setLoginVisible, isAuthenticated }) => {
  ...
  useEffect(() => {
    if (countryCode)
      api
        .get(
          `/browse?tag=data+analysis+-+available+tools&badges=true&ps_country_iso_code_a2=${countryCode}`
        )
        .then((d) => {
  ...
  return (
    <>
      ...
      <p>
        <Trans>Description - Section 4 - Available Tools</Trans>
      </p>
