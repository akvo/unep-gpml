      ...
      try {
        ...
        const updateLayer = await Promise.all(
          response.data.data.map(async (d) => {
            if (
              (d.attributes.arcgislayerId === 'Final_manufactured_plastic_goods___value__export__V2_WFL1' || 
              ...
              try {
                ...
                console.error(`Error fetching values for layer ${d.attributes.arcgislayerId}:`, valueError);
                return d;
              }
            }
            return d;
