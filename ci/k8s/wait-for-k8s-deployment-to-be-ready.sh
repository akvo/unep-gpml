...
while [ $(( $(date +%s) - 300 )) -lt ${starttime} ]; do
    ...
    consumer_status=`kubectl get pods -l "unep-gpml-version=$TRAVIS_COMMIT,run=unep-gpml" -o jsonpath='{range .items[*].status.containerStatuses[*]}{@.name}{" ready="}{@.ready}{"\n"}{end}'`

    old_consumer_status=`kubectl get pods -l "unep-gpml-version!=$TRAVIS_COMMIT,run=unep-gpml" -o jsonpath='{range .items[*].status.containerStatuses[*]}{@.name}{" ready="}{@.ready}{"\n"}{end}'`
